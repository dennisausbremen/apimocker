...
apiMocker.createServer = (options = {}) => {
  ...
  let saveBody;
  if (options.proxyURL || apiMocker.options.allowAvoidPreFlight) {
  ...
  if (apiMocker.options.allowAvoidPreFlight) {
    apiMocker.middlewares.push(
      bodyParser.json({
        strict: false,
        verify: saveBody,
        ...
        if (regexpTest) {
          ...
          if (typeof req.body !== 'string') {
            ...
            if (Object.keys(searchBody).length > 0) {
              ...
              // TODO: This is a hack!
            } else if (switchObject.key !== switchObject.switch) {
              searchBody = getRequestParam(req, switchObject.key)
            }
          }
...
apiMocker.start = (serverPort, callback) => {
  ...
  if (apiMocker.options.staticDirectory && apiMocker.options.staticPath) {
    const staticDir = apiMocker.options.staticDirectory
    const isAbsolutePath = staticDir.startsWith('/')
    const absOrRelDir = (isAbsolutePath) ? staticDir : path.join(process.cwd(), staticDir)
    apiMocker.express.use(
      apiMocker.options.staticPath,
      express.static(absOrRelDir),
    );
  }
